// Time-stamp: <2014-08-05 19:28:33 scinart>
// created at 2014-08-05 18:57:40
// cf244d2D.cc
...
int main()
{
    ...
    REP_1(i,n-2)
    {
        if((sa[i]<s1len && sa[i+1]>=s1len+1) ||
           (sa[i]>=s1len+1 && sa[i+1]<s1len))
        {
            int llcp = KoAluru::lcp_range(i,i+1);
            {
                int next = KoAluru::lcp_range(i,i+2);
                int prev = KoAluru::lcp_range(i-1,i+1);

                if(next>=llcp || prev>=llcp)
                {
                ...
                {
                    int update = max(prev+1,next+1);
                    checkmin(curmin,update);
                }
            }
        }
    }
