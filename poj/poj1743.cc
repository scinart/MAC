//
// Time-stamp: <2014-12-18 20:45:03 scinart>
...
bool valid(int x)
{
    ...
    REP_1(i,n-2)
    {
        if(KoAluru::lcp_range(i,i+1)>=x)
        {
            if(in)
            {
                checkmax(mi,KoAluru::lcp_range(i+1,i+1));
                checkmin(mj,KoAluru::lcp_range(i+1,i+1));
            }
            else
            {
                mi=max(KoAluru::lcp_range(i,i),KoAluru::lcp_range(i+1,i+1));
                mj=min(KoAluru::lcp_range(i,i),KoAluru::lcp_range(i+1,i+1));
                in=true;
            }
        }
        else
