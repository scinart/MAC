// Time-stamp: <2014-08-07 20:49:02 scinart>
...
SegmentTree* pst;
int lower_bound (int last);
int val;
int main()
{
// 杭电G++3.0.x跪C++过也就罢了，北大你G++4.4凭什么跪我啊。。。。
#ifdef ECLIPSE
    freopen("in.txt", "r", stdin);
#endif
    int n;
    REP(i,maxn)INPUT[i]=1;
    while(scanf("%d", &n)!=EOF)
    {
        REP(i,n)
        {
        ...
        {
            val = POS[i]+1;
            int lb = lower_bound(st.query_head);
            st.update(lb,lb+1,-1);
            ANS[lb] = iVAL[i];
        }
...
int lower_bound (int which_node)
{
    int lbound;
    int rbound;
    int mid;
    ...
    do
    {
        pst->pushDown(which_node);//necessary.
        lbound = L[which_node];
        rbound = R[which_node];
        mid = (lbound+rbound)>>1;
        lnode = pst->indexof(lbound,mid);
        rnode = pst->indexof(mid,rbound);
        if(rbound-lbound==1)
        {
            break;
        }
