// Time-stamp: <2014-08-06 20:53:33 scinart>
// created at 2014-08-06 14:16:27
// poj2528.cc
...
//        EACH(it,ina)si.insert(*it);
//        EACH(it,inb)si.insert(*it);
        EACH(it,ina)discretization[*it]=1;
        EACH(it,inb)discretization[*it]=1;
        int jj=0;
//        EACH(it,si)discretization[*it]=jj++;
        EACH(it,discretization)it->second=jj++;
        SegmentTree st(jj);
        REP(i,n)
        {
            st.update(discretization[ina[i]], discretization[inb[i]]+1, i+1);
        }
        set<int> ans;
        REP(i,jj)
        {
            ans.insert(st.query(i,i+1));
        }
        printf("%lu\n", ans.size()-(int)(ans.find(0)!=ans.end()));
    }
