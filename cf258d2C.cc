// Time-stamp: <2014-07-25 01:55:05 scinart>
// created at 2014-07-24 23:59:28
// cf2582C.cc
...
int main()
{
    ...
    while(cases--)
    {
        ...
        vector<ll> need(4,0);
        pre[0] = k-(d1+d2+d2);
        pre[1] = k-(2*max(d1,d2)-min(d1,d2));
        pre[2] = k-(d1+d2);
        pre[3] = k-(d1+d1+d2);

        need[0]= d1+d1+d2;
        // need[1]= max(d1,d2) + (max(d1,d2)-min(d1,d2));
        // need[2]= need[1];
        need[1] = d1+d2;
        need[2] = max(d1,d2) + (max(d1,d2)-min(d1,d2));
        need[3]= d1+d2+d2;

        REP(i,4)
        {
            if(pre[i]>=0 && pre[i]%3==0)
            {
                ans[i]=need[i];
            }
        }
