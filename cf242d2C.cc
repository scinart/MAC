// Time-stamp: <2014-08-26 16:01:20 scinart>
// created at 2014-08-26 15:23:16
// cf242d2C.cc
...
int dodo(int n)
{
    ...
    for(int i=2; i<=n; i++)
    {
        ...
        int rm=n%i;
        st.update(0,i,bv);
        if(rm>0)st.update(0,rm+1,1);
    }
//    REP_1(i,n)cout<<i<<':'<<st.query(i,i+1)<<' ';cout<<'\n';
    REP_1(i,n)
    {
        int q=st.query(i,i+1);
        if(q%2==1)
        {
            ans^=i;
        }
