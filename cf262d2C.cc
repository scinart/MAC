// Time-stamp: <2014-08-21 00:32:28 scinart>
// created at 2014-08-21 00:15:17
// cf262d2C.cc
...
inline Valtype mergeValVal(const Valtype& a,const Valtype& b){return min(a,b);}
inline const Valtype withValue(const Marktype& mark, const Valtype& val, int l, int r){return val+mark;}
...
public:
    void upd(int w)
    {
        int which_node = query_head;
        pushDown(which_node);//necessary.
        Valtype globalmin = VAL[which_node];
        int l=0,r=R[query_head];
        while(l+1<r)
        {
            int mid=(l+r)/2;
            if(query_aid(which_node,0,mid)==globalmin)
            {
                r=mid;
            }
            else
        ...
        this->update(l,min(l+w,R[query_head]),1);
    }
